...
import com.digitalcipher.spiked.construction.description.LearningFunctionDescription.STDP_SOFT
import squants.Time

/**
  * STDP learning function that has soft weight limits allowing the weights to be adjusted exponentially
  *
  ...
  */
case class StdpSoftLimitLearningFunction(inhibitionAmplitude: Double,
                                         inhibitionPeriod: Time,
                                         excitationAmplitude: Double,
                                         excitationPeriod: Time) {
  private val partition = "learning"
  override def toString: String = s"$partition; learning_type: ${STDP_SOFT.name}; " +
